...
document.addEventListener("DOMContentLoaded", function () {
  var buttons = document.querySelectorAll(".sub-information-tab__item");
  var contents = document.querySelectorAll(".sub-information-card");
  ...
  function activateTab(tabId) {
    ...
    var activeButton = document.querySelector(".sub-information-tab__item[data-tab=\"".concat(tabId, "\"]"));
  ...
  buttons.forEach(function (button) {
    button.addEventListener("click", function () {
      ...
      activateTab(tabId);
      history.pushState(null, "", "?tab=".concat(tabId)); // URLを更新
    });
  });

  // URLのクエリパラメーターからタブを取得
  var params = new URLSearchParams(window.location.search);
  var defaultTab = "tab1"; // デフォルトのタブ
  var tabFromUrl = params.get("tab") || defaultTab;
